...
--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
--//////////////////////////////////////////////////////////////// PRIDANIE PROCED⁄R, TRIGGROV, FUNKCIÕ atÔ.. /////////////////////////////////////////////////////////////////////
--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
--//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
begin
    ...
    if overenie = pouzivatel and overeniePreS = sledovatel and overenie <> overeniePreS then
        ...
        raise_application_error(-20000, 'Dan˝ puûivateæ: ' || pouzivatel || ' neexistuje preto nebolo moûnÈ pridaù sledovateæa');
    elsif sledovatel <> sledovatel then
         raise_application_error(-20000, 'Dan˝ puûivateæ: ' || sledovatel || '  neexistuje preto nebolo moûnÈ pridaù sledovateæa');
    elsif overenie = overeniePreS then
        raise_application_error(-20000, 'Poûivateæ sa nemÙûe sledovaù seba samÈho');
    else
...
begin
    ...
    if overeniePouzivatela = pouzivatel and datum_udalosti >= sysdate then
        ...
        raise_application_error(-20000, 'Dan˝ puûivateæ: ' || pouzivatel || ' neexistuje preto nebolo moûnÈ pridaù udalosù');
    elsif datum_udalosti < sysdate then
        raise_application_error(-20000, 'Nie je moûne pridaù d·tum udalosti lebo datum je uû "vËerajöÌ"');
    else
        raise_application_error(-20000, 'Chyba nastala niekde inde');
    end if;
end;
...
-- Procedura 4 - pridanie pouûÌvateæa    
...
BEGIN
    ...
    -- Generovanie n·hodn˝ch hodnÙt
      ...
      -- Vloûenie hodnÙt do tabuæky Pouzivatel
...
begin
    if :new.datum_udalosti < sysdate then
        raise_application_error(-20000, 'D·tum udalsti nesmie byù "vËerajöÌ"');
    end if;
end;
/
...
/
 ...
 begin
    ...
    raise_application_error(-20000, 'Pouûivateæ nemÙûe oznaËiù seba samÈho');
    end if;
 end;
 /
...
-- 2. V˝pis prÌspevkov od pouûÌvateæov sledovan˝ch urËit˝m pouûÌvateæom.
select p.* 
from prispevky p
...
-- 3. V˝pis pouûÌvateæov  s najviac sledovateæmi.
...
-- 4. V˝pis pouûÌvateæov snajviac prÌspevkami
...
-- 5. V˝pis pouûÌvateæov snajviac komentovan˝mi prÌspevkami.
...
-- 6.V˝pis pouûÌvateæov s prÌspevkami s najviac reakciami.
...
-- 7. V˝pis pouûÌvateæov, ktorÌ zaËali niekoho sledovaù minul˝ t˝ûdeÚ.
...
-- 8.VypÌsaù poËet registrovan˝ch pouûÌvateæov v danom obdobÌ
select count(pouzivatelske_meno) as pocet_registrovanych 
from pouzivatel
where datum_registracie between '27.03.2001' and '31.03.2022'; 

--9. VypÌsaù poËet registrovan˝ch pouûÌvateæov pre jednotlivÈ roky.
...
-- 10. V˝poËet prÌrastku sledovateæov pre jednotliv˝ch pouûÌvateæov za posledn˝ch 365 dnÌ.
...
--11. V˝poËet priemernÈho roËnÈho prÌrastku sledovateæov pre jednotliv˝ch pouûÌvateæov
SELECT v.pouzivatelske_meno,
       CASE WHEN EXTRACT(YEAR FROM SYSDATE) - EXTRACT(YEAR FROM datum_registracie) > 0 -- to mi zabezpeci pouzivatelov ktorÌ s˙ registrovanÌ viac ako jeden rok
...
--12.V˝pis najkomentovanejöÌch prÌspevkov
...
-- 13. V˝pis prÌspevkov s najviac reakciami danÈho typu
...
--14.V˝pis najfrekventovanejöie pouûÌvanÈhoÑtaguì vdanom Ëasovom obdobÌ.
...
-- 15. V˝pis Ñtagovì, ktorÈ za poslednÈ 3 mesiace boli pouûitÈ menej ako 5 kr·t alebo neboli pouûitÈ vÙbec
...
-- 16.V˝pis najaktÌvnejöÌch pouûÌvateæov vzhæadom na prÌspevky inÈho pouûÌvateæa 
--(ktor˝ pouûÌvateæ najviac komentoval/dal najviac reakciÌ na prÌspevky nejakÈho pouûÌvateæa
...
-- 17.V˝pis prÌspevkov, ktorÈ obsahuj˙ menej ako 5 tagov
SELECT *
FROM prispevky
WHERE id IN (
    SELECT id
...
-- 18. V˝pis prÌspevku s najviac reakciami pre kaûdÈho pouûÌvateæa
...
-- 19.V˝pis prÌspevku snajviac koment·rmi pre kaûdÈho pouûÌvateæa -- asi vacsina udajov kde je reakcia tak tam je aj komentar
...
-- 20. V˝pis prÌspevkov, ktorÈ nemaj˙ ûiadny koment·r
...
-- 21. V˝pis prÌspevkov, ktorÈ boli pridanÈ v piatok a cez vÌkend
SELECT *
FROM prispevky
WHERE TO_CHAR(cas_pridania_prispevku,'D') IN (5,6,7);


-- 22.V˝pis prÌspevkov, vktor˝ch nebol oznaËen˝ ani jeden pouûÌvateæ.
...
-- 23. V˝pis udalostÌ s najviac ˙ËastnÌkmi
...
-- 24. V˝pis  najprestÌûnejöÌch  udalostÌ  (podæa  pouûÌvateæov,  ktorÌ  sa  z˙Ëastnili  (prestÌû pouûÌvateæa sa r·ta podæa poËtu sledovateæov)
...
-- 25. V˝pis udalosti, ktor˝ch sa z˙Ëastnilo viac ako 5 æudÌ.
select count(p.odpoved_ucasti) as pocet_Zucastneni_sa, p.pouzivatelske_meno as clovek_organizujuci_udalost,p.datum_udalosti 
from udalosti u
join pozvani p on(u.pouzivatelske_meno = p.pouzivatelske_meno and p.odpoved_ucasti = 'Y')
group by p.pouzivatelske_meno, p.datum_udalosti
...
-- 26. V˝pis udalostÌ, ktorÈ sa bud˙ konaù bud˙ci mesiac
V˝pis udalostÌ, ktorÈ sa bud˙ konaù bud˙ci mesiac.
select *
from udalosti 
where to_char(datum_udalosti,'MM') = to_char(add_months(sysdate, 1), 'MM');

-- 27. V˝pis poËtu udalostÌ pre kaûdÈho pouûÌvateæa, ktor˝ch sa z˙Ëastnil minul˝ rok.
